...
export class InvoiceTableResultComponent implements OnInit, OnApplicationEvent {
    ...
    async load(event: number = 1) {
        const invoices = await firstValueFrom(
            this.invoiceService.search(this.archived, this.logicalDelete, event, this.pageSize, this.sort, this.bookmarked),
        );
        const attachments = await firstValueFrom(this.fileService
            .findByIds(invoices.content.map(u => u.invoiceUploadId)
                .filter(u => u?.length), false));
        const thumbs = await firstValueFrom(this.fileService
            .findByIds(attachments.map(u => u.thumbnailId)
                .filter(u => u?.length), true));
        for (const invoice of invoices.content) {
            if (invoice.invoiceUploadId?.length) {
                const attachment = attachments.find(a => a.id === invoice.invoiceUploadId);
                const thumb = thumbs.find(t => attachment?.thumbnailId === t.id);
                invoice.transientThumbnail = thumb;
            }
        }
