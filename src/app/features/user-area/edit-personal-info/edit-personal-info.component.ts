...
export class EditPersonalInfoComponent implements OnInit {
  ...
  set countryCode(c: string) {
    this.form.get('countryCode').patchValue(c);
  }
  ...
  async ngOnInit() {
    await Promise.all([
      ...
      (async () => {
        console.log(' load signature');
        if (this?.currentPersonalInfo?.signatureUploadId) {
          const signature = await firstValueFrom(
            this.fileService.toDownloadLink(
              this.fileService.getDownloadUrl(this.currentPersonalInfo.signatureUploadId),
            ),
          );
    ...
    this.form = this.fb.group({
      ...
      countryCode: new UntypedFormControl(
        { value: this.currentPersonalInfo?.countryCode, disabled: !this.hasRoleAdmin },
