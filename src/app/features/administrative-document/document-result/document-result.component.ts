...
export class DocumentResultComponent implements OnInit, OnDestroy, OnApplicationEvent {
  ...
  async load(event: number = 1) {
    const docs = await firstValueFrom(
      this.administrativeDocumentService.search(this.searchCriteria, event, this.pageSize),
    );
    const attachments = await firstValueFrom(
      this.fileService.findByIds(
        docs.content.map((u) => u.attachmentId).filter((u) => u?.length),
        false,
      ),
    );
    const thumbs = await firstValueFrom(
      this.fileService.findByIds(
        attachments.map((a) => a.thumbnailId).filter((a) => a?.length),
        true,
      ),
    );
