...
export class DocumentResultComponent implements OnInit, OnDestroy, OnApplicationEvent {
    ...
    toggleBookmark($event: MouseEvent, doc: AdministrativeDocument) {
        $event.stopPropagation();
        this.administrativeDocumentService.toggleBookmarked(doc.id).subscribe((_) => {
            this.toastService.showSuccess('Bookmark toggled. Reload...');
            doc.bookmarked = !doc.bookmarked;
            const index = this.adminDocuments.content.findIndex((d, idx) => d.id && d.id === doc.id);
            if (index !== -1) {
                const newArray =
                    this.bookmarked && !doc.bookmarked
                        ? this.adminDocuments.content.filter((x) => x.id !== doc.id)
                        : this.adminDocuments.content.map((item, i) => (i === index ? doc : item));
                this.adminDocuments.content = newArray;
            }
        });
    }
    async load(event: number = 1) {

        const docs = await firstValueFrom(this.administrativeDocumentService
            .search(this.searchCriteria, event, this.pageSize));
        const attachments = await firstValueFrom(this.fileService
            .findByIds(docs.content.map(u => u.attachmentId)
                .filter(u => u?.length), false));
        const thumbs = await firstValueFrom(this.fileService.findByIds(attachments.map(a => a.thumbnailId).filter(a => a?.length), true));
        ...
        for (const doc of docs.content) {
            const attachment = attachments.find(a => a.id == doc.attachmentId);
    ...
    addOrEdit(ad: AdministrativeDocument = { title: '' }) {
        let ref = this.modalService.open(DocumentEditorComponent, { size: 'xl', keyboard: false, backdrop: 'static' });
