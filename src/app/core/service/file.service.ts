...
export class FileService {
    ...
    getCorrelationLinks(): Observable<Record<string, string | null>> {
        return this.http.post<Record<string, string | null>>(`${this.basePath}/correlation-links`, {});
    }
    ...
    findByIds(ids: string[], withThumb = false): Observable<FileUpload[]> {
        if (!ids.length) {
            return of([]);
        }
        ...
        params.append('withThumb', withThumb + '');
    ...
    async downloadBulk(ids: string[]) {
        if (isPlatformBrowser(this.platformId)) {
            const link = await firstValueFrom(this.http
                .post(`${this.basePath}/download-bulk`, ids, {
                    observe: 'response',
                    responseType: 'blob' as 'json',
                },)
                .pipe(
    ...
    static isXML(contentType?: string): boolean {
